document.getElementById('fetch-cardio-videos').addEventListener('click', function() {
    const duration = document.getElementById('duration').value || 30; // Default to 30 minutes if no input
    const query = `cardio workout ${duration} minutes`;
    fetchCardioVideos(query);
});

function fetchCardioVideos(query) {
